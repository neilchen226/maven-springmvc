...
<form id="userinfo" class="layui-form layui-row">
	...
	<div class="layui-col-md6 layui-col-xs12">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<select name="jobname"  lay-verify="required" class="layui-input">
		...
		<div class="layui-form-item">
			<label class="layui-form-label">代号
				<span class="layui-badge-dot" style="position: relative; top: -3px; left: 5px;" title="必填项"></span>
			</label>
			<div class="layui-input-block">
				<input type="text" name="usernumber" value="${user.usernumber }" placeholder="请输入代号" lay-verify="required" class="layui-input">
			</div>
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">是否在职</label>
			<div class="layui-input-block">
				<input type="checkbox" name="iswork" lay-filter="changePass" lay-skin="switch" value="1" lay-text="是|否"<c:if test="${user.iswork ==1 }"> checked</c:if>>
		</div>
		<c:if test="${not empty user.userid }">
		<div class="layui-form-item">
...
<script>
	layui.use([ 'form', 'layer', 'element','upload','laydate' ], function() {
		...
		form.on('submit(saveUser)', function(data) {
			...
			$.post("/sys/user/save",data.field,function(res){
