...
public class BarCodeUtil {
	...
	static JBarcode getJBarcode() throws InvalidAtributeException {
		 ...
		 * //设置TextPainter barcode.setBarHeight(17); //设置高度
		 * barcode.setWideRatio(Double.valueOf(30).doubleValue());// 设置宽度比率
		 * barcode.setXDimension(Double.valueOf(2).doubleValue()); // 设置尺寸，大小
		 * 密集程度 barcode.setShowText(true); //是否显示文本 barcode.setCheckDigit(true);
		 * //是否检查数字 barcode.setShowCheckDigit(false); //是否显示检查数字
		 */
		if (jbc == null) {
			// 生成code39
	...
	protected static class CustomTextPainter implements TextPainter {
		...
		public void paintText(BufferedImage barCodeImage, String text, int width) {
			...
			// 绘制文本
			g2d.drawString(TEXT, 0, barCodeImage.getHeight() - (height / 10) - 2);
			// 绘制条形码
...
//	// 测试
	public static void main(String[] args) {
		String message="ASIN15-02-YW1810151708-0530EM";
		createBarcode(message, new File("D:\\" + message + ".png"), "test");
	}
}
